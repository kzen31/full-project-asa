...
@RequestMapping("/api/catering-dashboard")
public class CateringDashboardController extends HandlerController{
    private final CateringDashboardService cateringDashboardService;
    private final MainDashboardService mainDashboardService;


    @GetMapping("/widget")
    public ResponseEntity<List<RecordDashboard>> getAllInfoWidget () {
        return ResponseEntity.ok(cateringDashboardService.countEveryStatusCatering());
    }

    @GetMapping("/count-last-5-month")
    public ResponseEntity<List<CountByMonthDto>> getACountLastMonths () {
        return ResponseEntity.ok(mainDashboardService.getByMonthCatering());
    }
}
